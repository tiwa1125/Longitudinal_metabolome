...
load("~/data/top_100_merge_table_NDD.Rdata")
load("~/data/top_100_merge_table_ADHD.Rdata")
load("~/data/metabolite.info.Rdata")

# Subset metabolite.info where only the selected metabolites are included

selected.metabolite.info <- metabolite.info %>% filter(CHEM_ID%in%top_100_merge_table_NDD$var)

#selected.metabolite.info <- metabolite.info %>% filter(CHEM_ID%in%top_100_merge_table_ADHD$var)
...
p1 <- ggplot(sankey_data, aes(x = x,
  ...
  geom_sankey(flow.alpha = 0.5,
              ...
              smooth = 8,
              width = 0.1) +
  geom_sankey_text(size = 3, hjust = 1.2,  # Horizontal alignment: 0=left, 0.5=center, 1=right
                   vjust = 0.5,
                   color = "black") +
  theme_void() +
...
p <- ggdraw() +
  ...
  draw_plot(p2, 0.6, 0.15, .25, 0.655)
p
