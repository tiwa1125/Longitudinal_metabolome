...
SPLS
# Import excel sheet containing all metabolite information such as name and metabolic pathway (those metabolites with NO annotation are removed)
```{r}
##load the ID and names of all metabolites
metabolite.info <- read_xlsx("~/Data/Chemical_Annotation_Common.xlsx") %>%
  dplyr::select(CHEM_ID,PATHWAY_SORTORDER, CHEMICAL_NAME, SUB_PATHWAY, SUPER_PATHWAY)%>%
 filter(!grepl("XX", PATHWAY_SORTORDER))
# Add "X" to the start of each cell in the specific column
metabolite.info$CHEM_ID <- paste("X", metabolite.info$CHEM_ID, sep = "")

# Remove unknown metabolites where CHEM_ID contains "X9999" 
metabolite.info <- metabolite.info[!grepl("X9999", metabolite.info$CHEM_ID), ]

# Subset metabolite.info where only the selected metabolites are included

selected.metabolite.info <- metabolite.info %>% filter(CHEM_ID%in%top_100_merge_table_NDD$var)#%>%distinct()
